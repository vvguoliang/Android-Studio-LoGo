package logo.vvguoliang.com.logo.View;
...
import logo.vvguoliang.com.logo.Customcontrols.MyTextView;
import logo.vvguoliang.com.logo.Utils.DisplayUtils;

/**
 * Created by vvguoliang on 2017/2/16.
 */
...
//        typedArray.recycle();
    ...
    public void setLayout(String[] titles, int[] selectedImage, int[] unSelectedImage, int screenWidth, int mHeight,
        ...
        if (screenWidth > 720) {
            text_size = DisplayUtils.px2dip(context, 25) * 16 / 9;
        } else if (screenWidth <= 720 && screenWidth > 480) {
            text_size = DisplayUtils.px2dip(context, 25) * 16 / 9 - 5;
        } else if (screenWidth <= 480) {
            text_size = DisplayUtils.px2dip(context, 25) * 16 / 9 - 23;
        }
        if (titles != null && titles.length != 0) {
            ...
            for (int i = 0; i < titles.length; i++) {
                ...
                LayoutParams layoutLp = new LayoutParams(widthScale, LayoutParams.MATCH_PARENT);
                layoutLp.gravity = Gravity.CENTER;
                layout.setLayoutParams(layoutLp);

                ImageView image = new ImageView(context);
                LayoutParams imageLp = new LayoutParams(DisplayUtils.dip2px(context, 30), DisplayUtils.dip2px(context, 30));
